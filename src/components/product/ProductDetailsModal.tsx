...
import { X, ExternalLink, ShoppingCart, CheckCircle, AlertCircle } from 'lucide-react';
...
export function ProductDetailsModal({ productId, isOpen, onClose, onAddToCart }: ProductDetailsModalProps) {
          ...
          <div className="p-6">
            {loading && (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-3 text-gray-600 text-lg">Loading product details...</span>
              </div>
            )}
            ...
            {error && (
              <div className="text-center py-12">
                ...
                <div className="space-x-4">
                  <Button onClick={fetchProductDetails} className="bg-blue-600/90 hover:bg-blue-700/90 text-white px-6 py-3 rounded-xl">
                    Try Again
                  </Button>
                  <Button onClick={onClose} className="bg-gray-100/80 hover:bg-gray-200/80 text-gray-700 px-6 py-3 rounded-xl">
                    Close
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-gray-600">
                      ...
                      <span className="text-lg">Brand: {product.vendorName}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    {product.productAuthorized === 'True' && (
                      <Badge className="bg-green-100/80 text-green-700 border-green-200/50">Authorized</Badge>
                    )}
                    {product.productStatusCode && (
                      <Badge className="bg-blue-100/80 text-blue-700 border-blue-200/50">Status: {product.productStatusCode}</Badge>
                    )}
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-900">{product.productCategory || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Subcategory:</span>
                        <span className="text-gray-900">{product.productSubcategory || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Product Class:</span>
                        <span className="text-gray-900">{product.productClass || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">UPC Code:</span>
                        <span className="text-gray-900 font-mono text-sm">{product.upc || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Vendor Part Number:</span>
                        <span className="text-gray-900 font-mono text-sm">{product.vendorPartNumber || 'N/A'}</span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Vendor Number:</span>
                        <span className="text-gray-900">{product.vendorNumber || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Product Authorized:</span>
                        <span className="text-gray-900">{product.productAuthorized === 'True' ? 'Yes' : 'No'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Product Status:</span>
                        <span className="text-gray-900">{product.productStatusCode || 'Active'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Direct Ship:</span>
                        <span className="text-gray-900">{product.directShip === 'True' ? 'Yes' : 'No'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Discontinued:</span>
                        <span className="text-gray-900">{product.discontinued === 'True' ? 'Yes' : 'No'}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-6 border border-white/30">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h5 className="text-lg font-medium text-gray-700 mb-3">Pricing</h5>
                      {priceLoading ? (
                        ...
                        <div className="text-gray-600">Price available on request</div>
                      )}
                    </div>
                    <div>
                      <h5 className="text-lg font-medium text-gray-700 mb-3">Availability</h5>
                      {priceLoading ? (
                        ...
                        <div className="text-gray-600">Check availability for pricing</div>
                      )}
                    </div>
                  </div>
                </div>
